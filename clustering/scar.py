...
from clustering.Nystrom import eigendecompositionNystrom
...
class SCAR:
    ...
    def __init__(self, k, nn, alpha, theta=20, m=0.5, laplacian=0, n_iter=50, normalize=False, weighted=False,
                 verbose=False):
        """
    ...
    def __latent_decomposition(self, X):
        ...
        for it in range(self.n_iter):
            ...
            allowed_to_remove_per_node = (deg * self.m).astype(np.int_)
            prev_trace = trace
    ...
    def fit_predict(self, X):
        ...
        labels = KMeans(n_clusters=self.k, n_init=10).fit_predict(self.H)
